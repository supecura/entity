...
func (e *Entity) OnMessageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	first,commands := e.shift(sp)
	if strings.HasPrefix(first,"<@"+s.State.User.ID+">"){
		e.callBot(s,m,commands)
	}
	fmt.Printf("%20s %20s %20s > %s\n", m.Author.ID, time.Now().Format(time.Stamp), m.Author.Username, line)
}

func (e *Entity) SendPrivateMessage(discordSession *discordgo.Session, m *discordgo.MessageCreate, message string) {
